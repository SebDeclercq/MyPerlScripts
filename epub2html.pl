...
foreach my $item ( $xpc->findnodes('//opf:item') ) {
    if ( $item->findvalue('@media-type') =~ m/image/ ) {
        push( @imgfiles, $item->findvalue('@href') );
    }
}

$doc = $parser->parse_file( glob('*.ncx') );
$xpc = XML::LibXML::XPathContext->new($doc);
$xpc->registerNs( ncx => "http://www.daisy.org/z3986/2005/ncx/" );
my @htmlfiles;

### Fixes bugs of page order encountered with *.opf files
foreach my $navPoint ( $xpc->findnodes('//ncx:navPoint/ncx:content') ) {
    ### Disables all HTML anchors (file.html#blah)
    if ( $navPoint->findvalue('@src') =~ /html$/ ) {
        push( @htmlfiles, $navPoint->findvalue('@src') );
    }
}

### Get Book Title
my $title = $xpc->findvalue('//ncx:docTitle/ncx:text');

my $header = qq{
<html>
 <!-- This ugly file is automatically generated by epub2html.pl -->
 <!-- It is intended to produce a simple unique HTML file out of an EPUB file -->
